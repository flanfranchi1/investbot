...
def _(create_engine, db_uri, mo, pl):
    ...
    changes = (
        ...
        .with_columns(
            ...
            .str.strptime(pl.Date, format="%B %e, %Y", strict=True)
            .alias("date")
        )
        .drop("effective_date")
...
def _(changes, mo, pl):
    ...
    validate_order = (
        pl.when(pl.col("action") != previous_value)
        .then(pl.lit(True))
        .when(previous_value.is_null())
        .then(pl.lit(True))
        .otherwise(pl.lit(False))
        .alias("validate")
    )

    validation_df = changes.unique(
        pl.col("ticker", "date", "action"), keep="first"
    ).with_columns(validate_order)
    mo.ui.table(validation_df.group_by("validate").len())
    return (validation_df,)


@app.cell
def _(mo, pl, validation_df):
    not_validated = validation_df.filter(pl.col("validate") == False).select(
        pl.col("ticker")
    )
    tickers_for_analysis = validation_df.join(
        not_validated, on="ticker", how="inner"
    ).sort(pl.col("ticker", "date"))
...
def _(engine, mo, pl, tickers_for_analysis):
    tickers_list = (
        "'" + r"', '".join(list(set(tickers_for_analysis["ticker"].to_list()))) + "'"
    )
...
def _(mo, pl, tickers_for_analysis):
    ...
    mo.ui.table(
        tickers_for_analysis.filter(
            pl.col("date") >= dt.date.today() - dt.timedelta(days=364.25 * 5)
        )
    )
    return
