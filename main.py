...
if __name__ == "__main__":
    ...
    if sp500_data:
        tickers_chunk = (
            len(sp500_data) // 10 if len(sp500_data) > 10 else len(sp500_data)
        )
        ...
        for chunk in range(0, len(sp500_data), tickers_chunk):
            ...
            price_data_df = fetch_historical_data(target_tickers, start_date, end_date)
        ...
        if (
            ...
            tickers_and_dates_to_be_saved_df = (
                tickers_and_dates_df.merge(stacked_chuncks_df, how="left", on="ticker")
            ...
            try:
                ...
                logging.error(f"Failed to fetch data for {chunk}: {e} Skipping...")
